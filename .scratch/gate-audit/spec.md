...
八項實驗全數有「預期 vs 實際」紀錄，報告在 `docs/gate-audit-findings.md`。
注入物零殘留、`.env` 未動、全程未起 dev server。

**四道 gate 證明有紅燈能力**：E1（build 後掃 dist）、E4（tsc，且 `api/` 確實在範圍內，
章程的疑慮證否）、E6（快照鎖，改一字且長度不變仍紅）、E7（鏡像 `--check`，exit=1）。
E2／E8 如預期綠燈並各自實證一個缺口。

**兩項意外發現（章程未預期，都比原本設想的更嚴重）**：
1. `.env` 的 `GEMINI_API_KEY` 實測長度 53、**不是 `AIza` 前綴** → 文件版
   `grep -r "AIza" dist/` 對本專案的真實金鑰**根本無效**。這使章程的規則 (b)
   從「加分項」變成「唯一有效的那條」。列為 G1。
2. `api/_lib/http.ts:108` 的 log 消毒 regex 本身含 `AIza[0-9A-Za-z_-]+`
   → 若照章程字面把 `AIza` 掃描套到原始碼，會**開箱即紅**。腳本改用
   「前綴＋30 字以上金鑰字元」的形狀比對解決。列為 G4。

**偏離章程三處**（理由寫在 findings 的「偏離章程之處」節，請覆核）：額外加了規則 (c)
（`.env` 秘密值 × git 追蹤原始碼）、原始碼掃描改用形狀比對、第 5 段保留文件版語意
但加印相對 HEAD 的提醒。

另有 `tsconfig.json` 無 `include`／`exclude` 導致型檢吸進 8 個 `dist/assets/*.js`（G5）、
Vite `envPrefix` 預設值是後端金鑰不進 bundle 的唯一憑藉且零測試守護（G3）——
兩者都屬行為變更，未動，留給使用者拍板。
