...
Status: resolved（2026-07-30，證據 commit 7f8ff2e）
Blocked by: —（依 spec 順序第三票，最重）

## Resolution

- 三件一組全落地：cancel → CANCELLED reject（不二次 settle）；settled 守衛擋遲到增量；
  handler 取消分類靜默收尾（零寫入＋res.end 必呼叫）。
- 翻轉案 3 案照清單、先紅後綠有紀錄；清單外既有案例零修改（http.test 的 Record fixture
  補鍵除外，見 Comments「非翻轉的附帶調整」）。
- 新鎖 6 案（llm 1＋handler 4＋http 訊息 1）；故障注入 3 次精準紅。
- 673 → 679 全綠、假時鐘維持；strict 0 不變；gate 全綠。
