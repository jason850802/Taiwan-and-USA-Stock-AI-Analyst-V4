...
- [x] **CORE-01**: 後端建立 `api/_lib/` 共用層，提供同源/Origin 檢查、upstream fetch＋逾時、環境變數讀取與 upstream 錯誤映射
- [x] **CORE-02**: 所有 Vercel 函式採 Node.js runtime（與 @google/genai 相容），並各自設定 `maxDuration`（Gemini 120s、行情端點 30s），避免落入預設逾時被砍
- [x] **CORE-03**: `vercel dev` 能在本地單一 port 同時跑前端與 `/api/*` 端點

### 金鑰封存（KEY）

- [x] **KEY-01**: 移除 `vite.config.ts` 的 `GEMINI_API_KEY` define 注入，前端 bundle 不再內嵌金鑰
- [x] **KEY-02**: `GEMINI_API_KEY` 只存於 Vercel 環境變數、僅由後端讀取，且不使用 `VITE_` 前綴
- [x] **KEY-03**: build 後對 `dist/` 掃描金鑰（grep `AIza`）結果為 0，作為金鑰未外洩的驗收
- [x] **KEY-04**: 所有錯誤一律經後端分類層回傳 `{ code, message }`，不透傳 upstream 原文（避免金鑰或敏感資訊外洩）

### 代理端點（PROXY）

- [x] **PROXY-01**: 所有 Gemini 呼叫改走 `/api/gemini`，前端不再 `new GoogleGenAI`、不再持有金鑰
- [x] **PROXY-02**: Gemini 模型 ID 集中為環境變數（`GEMINI_MODEL_FAST` / `GEMINI_MODEL_THINKING`），production 預設用 stable ID，並提供 `MODEL_NOT_FOUND` 獨立錯誤分類與 fallback
...
- [x] **FE-01**: `services/gemini.ts` 改接 `/api/gemini`，回傳契約與既有行為不變
