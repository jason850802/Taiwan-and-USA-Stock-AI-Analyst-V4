...
<execution_arbitration>
## 第一輪執行裁定（2026-07-06，Codex 回報規則衝突後定案，權威覆蓋前面過嚴的措辭）

Codex 正確指出：鐵則 #2「StockChart 只准改 hex」與 Task 4「全站 emerald 歸零」衝突。裁定如下，
全站掃描的 18 處命中一次處理完（Task 4 依此執行）：

**A. 澄清 StockChart 雷區的真正邊界（覆蓋鐵則 #2 的過嚴措辭）**
雷區保護的是**繪製與互動的「邏輯」**：拖曳/縮放 state machine、recharts hooks
（useActiveTooltipDataPoints 等）、XAxis/YAxis/Bar/Line/Area/CandleStickShape 的結構與 props。
**文字標籤的 Tailwind 顏色 class（`text-red-400`/`text-emerald-400` 這種）不屬雷區**，可依語意替換。
StockChart 的 5 處(L107/181/205/226/232)全是 OHLC 資訊列/MACD 值/法人張數/方向指示的**文字色**，
且已是「紅=漲/正、綠=跌/負」：**`text-red-400`→`text-up`、`text-emerald-400`→`text-down`**（一對一換）。
recharts 繪製色（inline hex，如 Cell fill、stroke）仍照鐵則 #2 只換 hex。改完 diff 應只有 className
字串與 hex，仍不得出現拖曳/hooks/軸結構的邏輯變更。

**B. StockSearch（補進本期範圍，原 files_modified 漏列，已補）**
- L13-14 台股/美股市場標籤 `cls`：這是**市場別標籤、非漲跌語意**→改中性
  （`bg-surface-inset text-slate-300`，或用 `Badge variant="neutral"` 精神）。兩個都改中性。
- L148 搜尋按鈕：`bg-blue-600 … rounded-xl shadow-lg shadow-blue-600/20`→改用 `ui/Button variant="primary"`，
  或最低限度 token 化（`bg-accent`、`rounded-ctl`、移除彩色陰影）。

**C. ui/MarkdownReport（補進本期範圍）**
L23/L45 的 GO/好結果關鍵字綠色 `text-emerald-400`/`bg-emerald-500/*`/`border-emerald-500`
→**決策語意「好」→ ok token**（`text-ok`/`bg-ok-muted`/`border-ok`）。此檔另一處紅色（NO_GO/壞）
若存在則→ danger token。GO 仍綠、NO_GO 仍紅，只是改語意命名。

**D. EntryChecklist（本就在 Task 4 範圍，明確化）**
pass→ok、fail→danger、GO→ok、NO_GO→danger、WAIT→warn；漸層 `from-*-600 to-*-600`（L12/14/21/88）
改平面 tint（如 GO 卡 `bg-ok-muted border border-ok/40`、標頭 `bg-surface-inset`）；
L60/71/74 的 emerald 文字→ok（「通過/符合」語意）。`✓`/`✗` 是純符號非 emoji 狀態碼，保留。

**E. AnalysisResult L24** 漸層標頭→平面 `bg-ai/15`＋`text-ai`（本就在範圍）。

裁定後 Task 4 驗收的 grep（emerald-/rose-/shadow-(blue|purple|indigo)/gradient-to）在
App.tsx＋components/（含 ui/）應為 **0 命中**。
</execution_arbitration>
