...
- **production 模型預設用 stable ID，不用 preview**：`GEMINI_MODEL_FAST` 預設 `gemini-3.5-flash`（stable）；`GEMINI_MODEL_THINKING` 預設 `gemini-2.5-pro`（stable，**不是**現有程式碼裡的 `gemini-3.1-pro-preview`，因為 preview 模型可能在部署後 2 週內無通知下架）。兩者都可用環境變數覆寫成其他 ID（含 preview），但**程式碼裡的預設值必須是 stable**。
...
1. `api/_lib/config.ts`：
   ...
   - `export function getModelForMode(mode: 'fast' | 'thinking'): string`：`fast` 讀 `process.env.GEMINI_MODEL_FAST`，預設值 `"gemini-3.5-flash"`；`thinking` 讀 `process.env.GEMINI_MODEL_THINKING`，預設值 `"gemini-2.5-pro"`（stable，依 Claude's Discretion，不用 `gemini-3.1-pro-preview`）。
...
- 已嘗試本地啟動流程：優先 `vercel dev`；若失敗則改用 `vite dev` + 第二個 `vercel dev` 進程 + `vite.config.ts` 的 `server.proxy` 轉接 `/api`（{Claude 填入實際採用哪個方案、遇到什麼錯誤訊息（如有）})
  ...
  <how-to-verify>
1. 確認 `.env` 檔案（專案根目錄，git-ignored）內有真實的 `GEMINI_API_KEY`、`GEMINI_MODEL_FAST=gemini-3.5-flash`、`GEMINI_MODEL_THINKING=gemini-2.5-pro`（若沒有請先建立，內容可從既有 `.env` 的 `GEMINI_API_KEY` 沿用）。
...
- Gemini thinking 模式（`gemini-2.5-pro`）實際回應延遲的粗略測量結果（呼應 STATE.md 的「Phase 1 待測量」blocker）
</output>
