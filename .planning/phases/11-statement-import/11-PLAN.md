...
**Case G｜碎股**：MU 買 2.3209@861.7368（成交 2000.00、費 1.60 → cost 2001.60）；賣 2.3209@902.2611（成交 2094.05、費 1.68、**其他費用 0.05**）
→ **`realizedPnl = 2094.05−1.68−0.05−2001.60 = 90.72 USD`**
　（⚠️ 規劃期原寫 90.77，係手算時漏計帳單「其他費用」欄——美股賣出的 SEC/TAF 費，
　　實作已正確扣除，T6 真檔端到端驗證時發現並更正。此欄映射到 `ParsedTxn.tax`。）
...
5. ~~**既有 `calcUsFee` 0.008% vs 實際 0.08%**：本期不修（見 CONTEXT），需使用者另行決定。~~
   → **已解除（2026-07-23）**：使用者指示修正，費率改為 `0.0008`，並補上「與對帳單實收
   逐筆吻合」的費率鎖測試（1500→1.20／895→0.72／604→0.48／175.5→0.14／1643→1.31）。
   受影響的手算案例已重算：Case 3 買費 0.14→1.44、賣費 0.16→1.60、已實現 199.70→**196.96**；
   TWD 計價案例賣費 0.10→1.00、已實現 249.90→**249.00**。
   注意：D-06「匯入用帳單實數」的原則**不因此放寬**——帳單仍可能含折扣或最低收費，
   且各券商費率不同；但 PLAN 第 174 行「公式會差 10 倍」的舉例在修正後已不再成立。
